...
export default function CardPlayed({ listname, owner, image }) {
  return (
    <div className="m-CardPlayed">
      <div>
        {image !== "" ? (
          <img className="m-CardPlayed__image" src={image} />
        ) : (
          <svg viewBox="-20 -25 100 100" className="m-CardPlayed__svg">
            <path
              className="m-CardPlayed__svg__path"
      ...
      <div className="m-CardPlayed__info">
        <a className="m-CardPlayed__info__title">{listname}</a>
        <span className="m-CardPlayed__info__subtitle">De {owner}</span>
      </div>
    </div>
  );
}
